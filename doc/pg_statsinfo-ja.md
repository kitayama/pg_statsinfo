...
7.  [よくあるQ&A](#よくあるqa)
8.  [pg_statsinfo17からの変更点](#pg_statsinfo17からの変更点)
9.  [詳細情報](#詳細情報)
...
pg_statsinfo 18 の変更点は[こちら](#pg_statsinfo17からの変更点)をご覧ください。
...
スナップショットとして以下の統計情報を収集します。
  ...
  - [統計情報コレクタ](https://www.postgresql.jp/document/18/html/monitoring-stats.html)が収集する全ての情報。挿入
    / 更新 / 削除行数やバッファアクセス回数。
...
リポジトリDBのテーブル構成に関しては、「[pg_statsinfo v18 リポジトリDB構成](files/pg_statsinfo_v18_repository_infomation.xls)」を参照してください。
...
各アラート項目のアラートメッセージの内容については「[pg_statsinfo v18 レポート項目一覧](files/pg_statsinfo_v18_report_infomation.xls)」をご覧ください。
...
簡易レポート機能が出力するレポートの項目については「[pg_statsinfo v18 レポート項目一覧](files/pg_statsinfo_v18_report_infomation.xls)」をご覧ください。  
...
以下はPostgreSQL18のRHEL10のx86_64用のrpmをインストールする例です。
...
OS再起動時に/run/pg_statsinfoを作成させるために、/usr/lib/tmpfiles.dにファイルを作成します。
    ...
    # cat << EOF > /usr/lib/tmpfiles.d/pg_statsinfo-18.conf
...
postgres」の場合には、[pg_hba.conf](https://www.postgresql.jp/document/18/html/auth-pg-hba-conf.html)
...
[pg_stat_statements](https://www.postgresql.jp/document/18/html/pgstatstatements.html)
...
#### ログファイルの種類
  ...
  - CSVログファイル \
    PostgreSQLがCSV書式で出力するファイルです。CSV書式の詳細は[PostgreSQLのマニュアル](https://www.postgresql.jp/document/18/html/runtime-config-logging.html#RUNTIME-CONFIG-LOGGING-CSVLOG)をご覧ください。
...
<!-- end list -->
  ...
  - -r, --report=REPORTID  
    ...
    レポート種別IDとレポートの内容についての対応は「[pg_statsinfo v18
    レポート項目一覧](files/pg_statsinfo_v18_report_infomation.xls)」をご覧ください。
      - Summary
      - Alert
      - DatabaseStatistics
      - InstanceActivity
...
| pg_statsinfo.collect_index         | on                                          | スナップショット取得時にインデックス情報の取得の有効/無効を設定します。offにすると列情報が収集されなくなり、スナップショットサイズの削減が可能ですが、一部の情報がレポートされなくなります。[(*9)](#9_設定ファイル_レポート不可となる項目)  |
  ...
  - ##### 1_設定ファイル_メッセージレベル
    ...
    を区別しないことを除き、[log_min_messages](https://www.postgresql.jp/document/18/html/runtime-config-logging.html#RUNTIME-CONFIG-LOGGING-WHEN)
    と同じ優先順位です。
    disable \> alert \> panic \> fatal \> log \> error \> warning \>
    notice \> info \> debug
  - ##### 2_設定ファイル_書式指定  
    設定パラメータ
    [log_line_prefix](https://www.postgresql.jp/document/18/html/runtime-config-logging.html#GUC-LOG-LINE-PREFIX)
  ...
  - ##### 4_設定ファイル_接続文字列  
    ...
    詳細は[データベース接続制御関数](https://www.postgresql.jp/document/18/html/libpq-connect.html)のPQconnectdbを参照して下さい。
    この他、libpq
    が使用する環境変数の影響を受けますので、「[環境変数](https://www.postgresql.jp/document/18/html/libpq-envars.html)」も参照して下さい。
    パスワードの入力待ちにならないようにする必要があります。 パスワード認証が必要な場合には、PostgreSQL インスタンス起動ユーザに
    [.pgpass](https://www.postgresql.jp/document/18/html/libpq-pgpass.html)
    を設定し、パスワードの入力を自動化してください。 この際には、host を指定しない場合は hostaddr
    がホスト名として参照されます。詳細は「[パラメータキーワード](https://www.postgresql.jp/document/18/html/libpq-connect.html#LIBPQ-PARAMKEYWORDS)」を参照してください。
  - ##### 5_設定ファイル_SQLSTATE指定  
...
pg_statsinfo を使用する際には、以下の使用上の注意と制約があります。
  ...
  - 一部のタイムゾーンでのログのリポジトリ格納における注意点  
    ...
    を
    中国(PRC=CST+8)に設定したサーバからのログの時刻がリポジトリDBでは14時間進んで解釈されることになります。リポジトリDBで以下のように入力と異なる時刻が表示された場合には設定が必要です。設定方法は[こちら](https://www.postgresql.jp/document/18/html/datetime-config-files.html)を参照してください。
...
#### Q7. 簡易レポート機能でレポート生成を実行したところ、一部のレポート項目が表示されません。
  ...
  - Schema Information  
    ...
    [クライアント認証](https://www.postgresql.jp/document/18/html/client-authentication.html)
...
## pg_statsinfo17からの変更点

pg_statsinfo 17 からの変更点は以下の通りです。
...
[pg_ctl](https://www.postgresql.jp/document/18/html/app-pg-ctl.html),
[psql](https://www.postgresql.jp/document/18/html/app-psql.html),
[サーバの構成](https://www.postgresql.jp/document/18/html/runtime-config.html),
[統計情報コレクタ](https://www.postgresql.jp/document/18/html/monitoring-stats.html),
[システムカタログ](https://www.postgresql.jp/document/18/html/catalogs.html),
[pg_stat_statements](https://www.postgresql.jp/document/18/html/pgstatstatements.html),
[pg_stats_reporter](https://github.com/ossc-db/pg_stats_reporter)
