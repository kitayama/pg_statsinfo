...
$$
	SELECT
		...
		(io.read_bytes - COALESCE(pg_catalog.lag(io.read_bytes) OVER w, 0) ),
		...
		(io.write_bytes - COALESCE(pg_catalog.lag(io.write_bytes) OVER w, 0) ),
		...
		(io.extend_bytes - COALESCE(pg_catalog.lag(io.extend_bytes) OVER w, 0) ),
...
$$
	SELECT
		...
		pg_catalog.round(pg_catalog.avg(page_read)::numeric,3),
